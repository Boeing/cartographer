...
sensor::TimedPointCloudData CreateFakeRangeData(int from, int to) {
  ...
  for (int i = 0; i < kNumSamples; ++i) {
    ...
    result.ranges.push_back({Eigen::Vector3f{1., 2., 3.}, relative_time, 0.f});
  }
  return result;
}
...
TEST(RangeDataCollatorTest, SingleSensor) {
  ...
  const auto range_data_0 = CreateFakeRangeData(200, 300);
  auto output_0 = collator.AddRangeData(sensor_id, range_data_0);
  ...
  const auto range_data_2 = CreateFakeRangeData(-1000, 510);
  auto output_2 = collator.AddRangeData(sensor_id, range_data_2);
  EXPECT_EQ(common::ToUniversal(output_2.time), 510);
  EXPECT_EQ(output_2.origins.size(), 1);
  EXPECT_EQ(output_2.ranges.size(), kNumSamples);
...
TEST(RangeDataCollatorTest, TwoSensors) {
  ...
  ASSERT_EQ(output_1.ranges.size(), 2 * kNumSamples);
  EXPECT_NEAR(common::ToUniversal(
                  output_1.time +
                  common::FromSeconds(output_1.ranges[0].point_time.time)),
              -1000, 2);
