...
struct PointCloudSorter {
  ...
  bool operator()(const sensor::RangefinderPoint& lhs,
    ...
    const Eigen::Vector2f delta_lhs =
        lhs.position.head<2>().normalized();
    const Eigen::Vector2f delta_rhs =
        rhs.position.head<2>().normalized();
...
template <typename T>
struct CircleSorter {
  bool operator()(const Circle<T>& lhs,
                  const Circle<T>& rhs) {
...
template <typename T>
constexpr T calc_P(const T A, const T B, const T C, const T D, const T x, const T y, const T squared_sum)
{
    return A * squared_sum + B * x + C * y + D;
}

template <typename T>
constexpr T calc_P(const T A, const T B, const T C, const T D, const T x, const T y)
{
...
class CircleCostFunction {
 public:
  CircleCostFunction(const Eigen::Vector2f& point)
      : point_(point), squared_sum_(point_[0]*point_[0] + point_[1]*point_[1]) {}

  template <typename T>
  bool operator()(const T* const A, const T* const B, const T* const C, const T* const D, T* residual) const {
    T P = calc_P(A[0], B[0], C[0], D[0], T(point_[0]), T(point_[1]), T(squared_sum_));
    if (T(1.0) + T(4.0) * A[0] * P < T(0.0))
        return false;
...
typedef nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<int, DataSet, double>, DataSet, 2> KDTree;

}

std::vector<Circle<float>> DetectReflectivePoles(const sensor::PointCloud& point_cloud, const float radius)
{
    ...
    std::sort(sorted_range_data.begin(),
    ...
    for (size_t i=0; i<sorted_range_data.size(); ++i)
    {
        ...
        if (sorted_range_data[i].intensity <= 0)
            continue;
        ...
        fw_it = advance_and_wrap(fw_it, sorted_range_data.begin(), sorted_range_data.end());
        float angle = std::abs(angle_between(dir, fw_it->position.head<2>()));
        while (angle <= max_angle)
        {
            // assess candidate point
            const float distance_to_circumference = radius - (fw_it->position.head<2>() - position).norm();
            const float d2 = distance_to_circumference*distance_to_circumference;

            mse += d2;
            if (distance_to_circumference < 0)
                mse += d2;
            count++;
        ...
        // walk right
        auto bw_it = sorted_range_data.rbegin() + static_cast<int>(sorted_range_data.size()) - 1 - static_cast<int>(i);
        bw_it = advance_and_wrap(bw_it, sorted_range_data.rbegin(), sorted_range_data.rend());
        angle = std::abs(angle_between(dir, bw_it->position.head<2>()));
        while (angle <= max_angle)
        {
            // assess candidate point
            const float distance_to_circumference = radius - (bw_it->position.head<2>() - position).norm();
            const float d2 = distance_to_circumference*distance_to_circumference;
...
std::map<std::pair<int, int>, size_t> HoughCircles(const ProbabilityGrid& grid, const double radius, const size_t num_rotations)
{
    ...
    for (int j = 0; j < grid.limits().cell_limits().num_y_cells; ++j)
    {
        ...
        {
            const auto cc = grid.correspondence_cost_cells().at(grid.ToFlatIndex({i, j}));
            if (cc < occupied_value && cc != 0)
            {
                for (size_t rot=0; rot < num_rotations; ++rot)
                {
                    const double theta = 2.0 * M_PI * static_cast<double>(rot) / static_cast<double>(num_rotations);
                    const int x = i - static_cast<int>(radius * std::cos(theta) / grid.limits().resolution() + 0.5);
                    const int y = j - static_cast<int>(radius * std::sin(theta) / grid.limits().resolution() + 0.5);

                    if (x > 0 && x < grid.limits().cell_limits().num_x_cells
                            && y > 0 && y < grid.limits().cell_limits().num_y_cells)
                    {
...
std::vector<DataSet::Cell> DBScan(const std::vector<DataSet::Cell>& cells, const double min_cluster_distance, const size_t min_cluster_size)
{
    ...
    for (std::size_t i = 0; i < data_set.cells.size(); ++i)
    {
      ...
      const size_t num_matches =
          kdtree.radiusSearch(&query_pt[0], min_cluster_distance,
      ...
      for (std::size_t n = 0; n < ret_matches.size(); ++n) {
...

        {
          ...
          const size_t sub_num_matches = kdtree.radiusSearch(
              &sub_query_pt[0], min_cluster_distance, sub_ret_matches,
...
std::vector<Eigen::Array2i> FilterByComponentSize(const std::vector<Eigen::Array2i>& cells,
                                                  const ProbabilityGrid& grid,
                                                  const int radius_in_cells,
                                                  const size_t min_cell_count,
                                                  const size_t max_cell_count)
{
    ...
    for (size_t i = 0; i < cells.size(); ++i)
    {
        ...
        {
            const double r2 = static_cast<double>(radius_in_cells) * static_cast<double>(radius_in_cells);

            const int x_min = std::max(0, cell.x() - radius_in_cells);
            const int y_min = std::max(0, cell.y() - radius_in_cells);

            const int x_limit = std::min(cell.x() + radius_in_cells, grid.limits().cell_limits().num_x_cells - 1);
            const int y_limit = std::min(cell.y() + radius_in_cells, grid.limits().cell_limits().num_y_cells - 1);

            for (int jj = y_min; jj <= y_limit; ++jj)
            {
                for (int ii = x_min; ii <= x_limit; ++ii)
                {
                    const double _r2 = (ii - cell.x()) * (ii - cell.x()) + (jj - cell.y()) * (jj - cell.y());
                    if (_r2 <= r2)
                    {
                        const auto cc = grid.correspondence_cost_cells().at(grid.ToFlatIndex({ii, jj}));
                        if (cc < occupied_value && cc != 0)
                        {
        ...
        while (!to_visit.empty() && connected.size() <= max_cell_count)
        {
            ...
            {
                const std::pair<int, int> c(to_visit.front().first + n.first, to_visit.front().second + n.second);
                if (connected.find(c) == connected.end())
                {
                    if (!grid.limits().Contains({c.first, c.second}))
                        continue;

                   const auto cc = grid.correspondence_cost_cells().at(grid.ToFlatIndex({c.first, c.second}));
                   if (cc < occupied_value && cc != 0)
                   {
                        connected.insert(c);
                        to_visit.push(c);
        ...
        if (connected.size() >= min_cell_count && connected.size() < max_cell_count)
        {
...
float CircumferenceCoverage(const int x, const int y, const ProbabilityGrid& grid, const int radius_in_cells)
{
    const double r2 = static_cast<double>(radius_in_cells) * static_cast<double>(radius_in_cells);
    int n = 0;
    float coverage = 0;
    ...
    const int x_limit = std::min(x + radius_in_cells, grid.limits().cell_limits().num_x_cells - 1);
    const int y_limit = std::min(y + radius_in_cells, grid.limits().cell_limits().num_y_cells - 1);
...
std::vector<DataSet::Cell> ComputeKernel(const Eigen::MatrixXd& kernel, const ProbabilityGrid& grid, const float threshold)
{
        ...
        for (int ii = half_k; ii < x_limit; ++ii)
        {
            ...
            for (int k_jj = 0; k_jj < k_size; ++k_jj)
            {
                for (int k_ii = 0; k_ii < k_size; ++k_ii)
                {
                    ...
                    auto cc = grid.correspondence_cost_cells().at(grid.ToFlatIndex({e_ii, e_jj}));
                    if (cc < occupied_value && cc != 0)
                    {
...
std::vector<Eigen::Array2i> DetectCircles(const ProbabilityGrid& grid, const double radius)
{
    const int radius_in_cells = static_cast<int>(radius / grid.limits().resolution() + 0.5);
...

    {
        ...
        const double r2 = (radius / grid.limits().resolution()) * (radius / grid.limits().resolution());
    ...
    /*
    {
        ...
        uint32_t* pixel_data = reinterpret_cast<uint32_t*>(cairo_image_surface_get_data(surface));
    ...
    */
    ...
    const float free_space_cost_threshold = 0.30 * M_PI * radius_in_cells * radius_in_cells;
    ...
    const auto clusters = DBScan(filtered_cells, min_cluster_distance, min_cluster_size);

    std::vector<Eigen::Array2i> cluster_cells;
    for (const auto& c : clusters)
        cluster_cells.push_back({c.x, c.y});

    const size_t max_cell_count = static_cast<size_t>(2 * radius_in_cells * radius_in_cells);
    const size_t min_cell_count = static_cast<size_t>(radius_in_cells);

    const auto components = FilterByComponentSize(cluster_cells, grid, radius_in_cells, min_cell_count, max_cell_count);
...
//    const float D = circle.position.x() * circle.position.x() + circle.position.y() * circle.position.y() - circle.radius * circle.radius;
    ...
    //    const Eigen::Matrix<float, 3, 1> b(-xz_hat - x_hat * D, -yz_hat - y_hat * D, -z_hat - D);

    const Eigen::Matrix<float, 3, 1> x = A.lu().solve(b); //A.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(b);
    ...
    std::cout << "circle.x: " << circle.position.x() << " -> " << a_fit << std::endl;
    std::cout << "circle.y: " << circle.position.y() << " -> " << b_fit << std::endl;
    std::cout << "check D: " << x[2] << " -> " << D_check << std::endl;

    return Circle{0.f, static_cast<int>(circle.points.size()), circle.radius, Eigen::Vector2f{a_fit, b_fit}, circle.points};
}
...
//     computing moments
    ...
    {
        Xi = data.points[i].x() - data.position.x();   //  centered x-coordinates
        Yi = data.points[i].y() - data.position.y();   //  centered y-coordinates
        Zi = Xi*Xi + Yi*Yi;
...
Circle<float> FitCircle(const Circle<float>& circle)
{
    ...
    for (size_t i = 0; i < circle.points.size(); ++i)
    {
      ...
      auto auto_diff = new ceres::AutoDiffCostFunction<CircleCostFunction, 1, 1, 1, 1, 1>(cost_fn);
      problem.AddResidualBlock(auto_diff, nullptr, &A, &B, &C, &D);

      const double P = calc_P(A, B, C, D, static_cast<double>(circle.points[i][0]), static_cast<double>(circle.points[i][1]));
    ...
    const double D_min = calc_D(A, std::min(std::abs(B_min), std::abs(B_max)), std::min(std::abs(C_min), std::abs(C_max)));
    const double D_max = calc_D(A, std::max(std::abs(B_min), std::abs(B_max)), std::max(std::abs(C_min), std::abs(C_max)));
    ...
    return Circle<float>{static_cast<float>(summary.final_cost), static_cast<int>(circle.points.size()), circle.radius, Eigen::Vector2f{a, b}, circle.points};
}
