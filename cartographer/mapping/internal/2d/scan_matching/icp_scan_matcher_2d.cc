...
proto::ICPScanMatcherOptions2D CreateICPScanMatcherOptions2D(
  ...
  options.set_point_inlier_threshold(parameter_dictionary->GetDouble("point_inlier_threshold"));
  options.set_feature_inlier_threshold(parameter_dictionary->GetDouble("feature_inlier_threshold"));
  return options;
}
...
ICPScanMatcher2D::Result ICPScanMatcher2D::Match(
  ...
  const double point_inlier_d2 = options_.point_inlier_threshold() * options_.point_inlier_threshold();
  const double feature_inlier_d2 = options_.feature_inlier_threshold() * options_.feature_inlier_threshold();
...

  {
    ...
    for (size_t i = 0; i < included_features.size(); ++i) {
      const Eigen::Vector2d fp(features[included_features[i]].keypoint.position.x(),
...
ICPScanMatcher2D::Statistics ICPScanMatcher2D::EvalutateMatch(const Result& result, const sensor::RangeData& range_data) const
{
    // point match check
