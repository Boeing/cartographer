...
class StaticCost {
 public:
  explicit StaticCost(const double cost)
      : cost_(cost) {}
...
CircleFeatureIndex CreateIndexForCircleFeatures(const std::vector<CircleFeature>& circle_features)
{
    CircleFeatureIndex index;
    index.feature_set.data = circle_features;
    index.kdtree.reset(new CircleFeatureKDTree(3, index.feature_set, nanoflann::KDTreeSingleIndexAdaptorParams(10)));
...
proto::ICPScanMatcherOptions2D CreateICPScanMatcherOptions2D(
  ...
  options.set_nearest_neighbour_point_huber_loss(parameter_dictionary->GetDouble("nearest_neighbour_point_huber_loss"));
  options.set_nearest_neighbour_feature_huber_loss(parameter_dictionary->GetDouble("nearest_neighbour_feature_huber_loss"));
  options.set_point_pair_point_huber_loss(parameter_dictionary->GetDouble("point_pair_point_huber_loss"));
  options.set_point_pair_feature_huber_loss(parameter_dictionary->GetDouble("point_pair_feature_huber_loss"));
  options.set_unmatched_feature_cost(parameter_dictionary->GetDouble("unmatched_feature_cost"));
...
ICPScanMatcher2D::ICPScanMatcher2D(const Submap2D& submap, const proto::ICPScanMatcherOptions2D& options)
    : options_(options),
      ceres_solver_options_(CreateCeresSolverOptions()),
      limits_(submap.grid()->limits()),
      kdtree_(CreateRealIndexForGrid(*submap.grid())),
      circle_feature_index_(CreateIndexForCircleFeatures(submap.CircleFeatures()))
{
...
ICPScanMatcher2D::Result ICPScanMatcher2D::Match(
  ...
  for (size_t i = 0; i < features.size(); ++i)
  {
      if (circle_feature_index_.feature_set.data.empty())
      {
          ...
          auto cost_fn = new StaticCost(options_.unmatched_feature_cost() * options_.feature_weight() / std::sqrt(static_cast<double>(features.size())));
          auto auto_diff = new ceres::AutoDiffCostFunction<StaticCost, 2, 3>(cost_fn);
      ...
      {
          auto cost_fn =
                  new NearestFeatureCostFunction2D(
                      options_.feature_weight() / std::sqrt(static_cast<double>(features.size())),
                      features[i],
                      *(circle_feature_index_.kdtree));
          auto numeric_diff = new ceres::NumericDiffCostFunction<NearestFeatureCostFunction2D, ceres::CENTRAL, 2, 3>(cost_fn);
  ...
  for (size_t i = 0; i < point_cloud.size(); ++i) {
    ...
    auto cost_fn =
        new SingleNearestNeighbourCostFunction2D(
                options_.point_weight() / std::sqrt(static_cast<double>(point_cloud.size())),
                point,
                *kdtree_.kdtree);
    auto numeric_diff =
...

  {
    ...
    for (size_t i = 0; i < point_cloud.size(); ++i) {
      const Eigen::Vector2d point(point_cloud[i].position.x(), point_cloud[i].position.y());
  ...
  if (!circle_feature_index_.feature_set.data.empty())
  {
      ...
      for (size_t i = 0; i < features.size(); ++i)
      {
          const Eigen::Vector2d fp(features[i].keypoint.position.x(), features[i].keypoint.position.y());
          ...
          circle_feature_index_.kdtree->findNeighbors(result_set, &query_pt[0], nanoflann::SearchParams(10));

          result.pairs.push_back({world,
                                  {circle_feature_index_.feature_set.data[ret_index].keypoint.position.x(),
                                   circle_feature_index_.feature_set.data[ret_index].keypoint.position.y()}});
      }
  }
...
ICPScanMatcher2D::Result ICPScanMatcher2D::MatchPointPair(
...

  {
      ...
      for (size_t i = 0; i < features.size(); ++i)
      {
        ...
        else
        {
            ...
            result.pairs.push_back({world, {circle_feature_index_.feature_set.data[ret_index].keypoint.position.x(),
                                            circle_feature_index_.feature_set.data[ret_index].keypoint.position.y()}});
        }
      }
  }
...

  {
      ...
      for (size_t i = 0; i < point_cloud.size(); ++i) {
        ...
        result.pairs.push_back({world, {kdtree_.cells.cells[ret_index].x,
                                        kdtree_.cells.cells[ret_index].y}});
      }
  }
