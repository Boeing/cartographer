...
transform::Rigid3d PoseExtrapolator::odom(common::Time time) {
  if (odometry_data_.empty()) return transform::Rigid3d::Identity();
  return interpolate(odometry_data_, time);
}
...
PoseExtrapolator::Extrapolation PoseExtrapolator::ExtrapolatePose(
  ...
  if (cached_extrapolated_pose_.time != time) {
    ...
    transform::Rigid3d reference_odom =
        interpolate(odometry_data_, newest_timed_pose.time);

    // estimate odometry at the queried time
    transform::Rigid3d current_odom = interpolate(odometry_data_, time);
